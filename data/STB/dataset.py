...
from common.utils.transforms import cam2pixel, pixel2cam, world2cam
from common.utils.vis import vis_3d_keypoints, vis_keypoints
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, transform, mode):
        ...
        self.joint_type = {
            "right": np.arange(self.joint_num, self.joint_num * 2),
            "left": np.arange(0, self.joint_num),
        }
        ...
        for aid in db.anns.keys():
            ...
            focal, princpt = np.array(cam_param["focal"], dtype=np.float32), np.array(
                cam_param["princpt"], dtype=np.float32
            )
    ...
    def __getitem__(self, idx):
        ...
        joint_coord, joint_valid, rel_root_depth, root_valid = transform_input_to_output_space(
            joint_coord, joint_valid, rel_root_depth, root_valid, self.root_joint_idx, self.joint_type
        )
    ...
    def evaluate(self, preds):
        ...
        for n in range(sample_num):
            ...
            pred_joint_coord_cam[self.joint_type["right"]] = (
                pred_joint_coord_cam[self.joint_type["right"]]
                - pred_joint_coord_cam[self.root_joint_idx["right"], None, :]
            )
            pred_joint_coord_cam[self.joint_type["left"]] = (
                pred_joint_coord_cam[self.joint_type["left"]]
                - pred_joint_coord_cam[self.root_joint_idx["left"], None, :]
            )
            gt_joint_coord[self.joint_type["right"]] = (
                gt_joint_coord[self.joint_type["right"]] - gt_joint_coord[self.root_joint_idx["right"], None, :]
            )
            gt_joint_coord[self.joint_type["left"]] = (
                gt_joint_coord[self.joint_type["left"]] - gt_joint_coord[self.root_joint_idx["left"], None, :]
            )
