...
for img_path in img_path_list:
    ...
    for hand_type in ("right", "left"):
        ...
        t, R = np.array(cam_param["campos"][str(cam_idx)], dtype=np.float32).reshape(3), np.array(
            cam_param["camrot"][str(cam_idx)], dtype=np.float32
        ).reshape(3, 3)
        t = -np.dot(R, t.reshape(3, 1)).reshape(3)  # -Rt -> t

        # gt 3D joint coordinate
        joint = np.array(joints[str(capture_idx)][str(frame_idx)]["world_coord"], dtype=np.float32).reshape(
            -1, 3
        )  # world coordinate
        joint = np.dot(R, joint.transpose(1, 0)).transpose(1, 0) + t.reshape(
            1, 3
        )  # apply camera extrinsic to convert world coordinates to camera coordinates
        joint_valid = np.array(joints[str(capture_idx)][str(frame_idx)]["joint_valid"], dtype=np.float32).reshape(
            -1, 1
        )
        ...
        mesh = np.dot(R, mesh.transpose(1, 0)).transpose(1, 0) + t.reshape(
            1, 3
        )  # apply camera extrinsic to convert world coordinates to camera coordinates

        # fitting error
        fit_err = get_fitting_error(
            mesh, joint_regressor, joint, joint_valid, hand_type
        )  # error between GT 3D joint coordinates in camera coordinate system
        ...
        output = mano_layer[hand_type](
            global_orient=root_pose, hand_pose=hand_pose, betas=shape
        )  # this is rotation-aligned, but not translation-aligned with the camera coordinates
        ...
        trans = (
            np.dot(R, trans.reshape(3, 1)).reshape(1, 3)
            - root_joint
            + np.dot(R, root_joint.transpose(1, 0)).transpose(1, 0)
            + t / 1000
