...
# snapshot load
model_path = "./pretrained/InterNet_snapshot_%d.pth.tar" % int(args.test_epoch)
...
joint_coord[:, :2] = np.dot(
    inv_trans, np.concatenate((joint_coord[:, :2], np.ones_like(joint_coord[:, :1])), 1).transpose(1, 0)
).transpose(1, 0)
...
print(f"Right hand exist: {right_exist}")
print(f"Left hand exist: {left_exist}")

# visualize joint coord in 2D space
filename = "demo/result_2d.jpg"
...
# To make x,y, and z in real unit (e.g., mm), you need to know camera intrinsic and root depth.
# The root depth can be obtained from RootNet (https://github.com/mks0601/3DMPPE_ROOTNET_RELEASE)
filename = "demo/result_3d.png"
